...
///
/// Workflow:
///   1. Phase 2 runs pin detection at symbol edges
///   2. User sees detected pins (orange circles)
///   3. Click near a symbol edge to add a new pin
///   4. Click existing manual pin to select, press Del to delete
///   5. Click OK to proceed to Phase 3 (wire tracing)
/// </summary>
public sealed class PinEditor : Form
{
    private readonly Bitmap _image;
    private readonly List<Rectangle> _symbolBoxes;  // YOLO + manual boxes
    private readonly List<WireTracer.DetectedPin> _detectedPins;  // from auto-detection
    private readonly List<WireTracer.DetectedPin> _manualPins = new();  // user-added pins
        ...
        IReadOnlyList<WireTracer.DetectedPin> detectedPins)
    {
        ...
        _symbolBoxes = yoloHits.Select(h => h.Bounds).Concat(manualBoxes).ToList();
        ...
        var clearBtn = new Button { Text = "Clear Manual Pins", AutoSize = true, Margin = new Padding(3) };
        clearBtn.Click += (_, _) =>
        {
        ...
        _countLabel = new Label { Text = "0 manual pins", AutoSize = true, Margin = new Padding(12, 8, 3, 3) };
        ...
        _statusLabel = new Label
        {
            ...
            Text = "Click near symbol edge to add pin - Orange=detected, Lime=manual - Wheel=zoom - Middle/right-drag=pan",
        };
        bottom.Controls.Add(_statusLabel, 0, 0);

        var okBtn = new Button { Text = "OK - Proceed to Wire Tracing", AutoSize = true, Margin = new Padding(3) };
    ...
    {
        _countLabel.Text = $"{_manualPins.Count} manual pin(s), {_detectedPins.Count} detected";
    }
    ...
    {
        // Find which symbol box and edge the point is closest to
        const int maxDist = 30;  // max distance from edge to place pin
    ...
    {
        // Snap the point to be exactly on the symbol edge
        return side switch
        {
    ...
    private void OnCanvasMouseDown(object? sender, MouseEventArgs e)
    {
        if (e.Button == MouseButtons.Left)
        {
            ...
            // Check if clicked on an existing manual pin
            const int pinRadius = 8;
            for (int i = _manualPins.Count - 1; i >= 0; i--)
            {
                var loc = _manualPins[i].Location;
                int distSq = (imgPt.X - loc.X) * (imgPt.X - loc.X) + (imgPt.Y - loc.Y) * (imgPt.Y - loc.Y);
                if (distSq <= pinRadius * pinRadius)
                {
                    _selectedManual = i;
                    _canvas.Invalidate();
                    return;
            ...
            // Try to add a new pin near a symbol edge
    ...
    private void OnCanvasPaint(object? sender, PaintEventArgs e)
    {
        ...
        using var penBox = new Pen(Color.Magenta, 1f);
        foreach (var box in _symbolBoxes)
        {
        ...
        // Draw detected pins (orange)
        using var brushDetected = new SolidBrush(Color.OrangeRed);
        using var penDetected = new Pen(Color.DarkOrange, 1f);
        const int pinRadius = 5;
        foreach (var pin in _detectedPins)
        {
            var sp = ImageToScreen(pin.Location);
            int sr = (int)(pinRadius * scale);
            sr = Math.Max(3, sr);
            g.FillEllipse(brushDetected, sp.X - sr, sp.Y - sr, sr * 2, sr * 2);
            g.DrawEllipse(penDetected, sp.X - sr, sp.Y - sr, sr * 2, sr * 2);
        }

        // Draw manual pins (lime green)
        using var brushManual = new SolidBrush(Color.Lime);
        using var penManual = new Pen(Color.DarkGreen, 1f);
        using var penSelected = new Pen(Color.Yellow, 2f);
